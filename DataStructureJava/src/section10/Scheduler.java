...
public class Scheduler {
	...
	public void processCommand() {
		...
		while (true) {
			...
			if (command.equalsIgnoreCase("addevent")) {
				...
				Arrays.sort(events, 0, n); // 추상클래스 인터페이스 4강 내용
	...
	private void handleShow() {
		...
		for (int i = 0; i < n; i++) {
			if (events[i].isRelevant(theDate)) { // isRelevant static binding
				System.out.println(events[i].toString()); // toString 메서드는?? Object클래스에서 상속받아서 괜찮음.
			}
		}
	}
	...
	private void handleList() {
		for (int i = 0; i < n; i++) {
			System.out.println("   " + events[i].toString()); // events객체 > dynamic binding 반대는? static binding(컴파일러에서
																// 결정)
		}
	}
